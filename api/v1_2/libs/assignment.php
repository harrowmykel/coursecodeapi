<?php 
	class Assignment{
		...
		public function sendAssignment($lock){
			...
			if (queryNum($quer)>0){
				...
				$querhb="SELECT * FROM assign_teach WHERE ass_code='$lock' AND auto_mark=1";
				$arr=array();
				$retyu=saveFile__($lock);
				$def=$defi=$retyu[1];
				$def=getUploadedName($defi);
				array_push($arr, $retyu[0]);
				array_push($arr, $def);				
				if (queryNum($querhb)>0){
					$grade=$this->mark_this_Assignment($lock, $defi);
				}
				$qury="INSERT INTO assignment (id, username, file_name, ass_code, time, grade) VALUES (NULL, '$user', '$defi', '$lock', $time, $grade)";
				queryMysql($qury);
				return $arr;
			}
			return emptyArray();
		...
		public function mark_this_Assignment($link, $ass_name){
			$defi=$ass_name;
			$lock=$link;
    		$link__=$this->getLoc()."assignment/"."$lock"."/"."$defi";
    		$link____=$this->getLoc()."marking_scheme/$lock".".xlsx";
    			if((!file_exists($link__)) || (!file_exists($link____))){
    			return -1;
    		}

    		$student__=$Reader = new SpreadsheetReader($link____);
			$teacher__=$Reader2 = new SpreadsheetReader($link__);
		...
		public function changeLock($lock, $new){
			...
			if ($nu>0){
				$this->moveA($lock, $new);
				$olp=$this->getLoc()."marking_scheme/$lock.xlsx";
				$nep=$this->getLoc()."marking_scheme/$new.xlsx";
		...
		public function createLock(){
			...
			$time=time();
			$link=$dell=$this->getUnUsed();
			$this->createAssFile($dell);			
			$autk=isset($_FILES['assignment'])?1:0;
			$query="INSERT INTO assign_teach (id, username, title_, ass_code, time, method_, deactivation_time, deleted_, deleted_time, deactivated_,deactivated_time, auto_mark) VALUES (NULL, '$username', '$title_', '$dell', $time, '', $deac, 0,0, 0,0, $autk)";
			queryMysql($query);
			if($autk==1){
				$dfg=$this->getLoc()."marking_scheme/$link";
	...
	function getSpreadSheet_($link){
		...
		$excel->initialize();
		$row = array("S/N", "Name", "File Name", "File Type", "File Size", "File Upload Time", "grade");
		$excel->addRow($row);
		for($i = 0; $i<count($body); $i++) {
			$arr=$body[$i][0];
			$ii=$i+1;		
			$row = array($ii, $arr['name'], $arr['file_name'], $arr['file_type'], $arr['file_size'], formatThisForHr($arr['file_mod']), $arr['grade']);
			$excel->addRow($row);
		}
		$excel->finalize();
