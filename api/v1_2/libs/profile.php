<?php 
	...
	function checkAppid(){
		...
		$time=time();
		$size=0;
		$quer="SELECT * FROM app_id_storage WHERE app_id='$ket' AND expiry_time>$time";
		if(querynum($quer)>0){
			$res=queryMysql($quer);
			$arr=$res->fetch_array(MYSQLI_ASSOC);
			$size_=$arr['size__'];
			$mem_use=$arr['memory_use'];
			if(($size_-$mem_use)>0){
				if(isset($_FILES)){
					...
					$n_size=$mem_use+$size;
					$quer="UPDATE app_id_storage SET memory_use=$n_size WHERE app_id='$ket'";
					queryMysql($quer);
				};
	    		return true;
			}		
